...
public class SwaggerClientRunnable implements Runnable {
    ...
    private ConcurrentLinkedQueue<String[]> latencyRecords;
    ...
    public void run() {
        ...
        for (int i = 0; i < this.numRequestsPerThread; i++) {
            try {
                ...
                // keep record of this thread's latency, format:
                // start time, request type (ie POST), latency, response code
                String[] record = new String[] {String.valueOf(start_time),
                        "POST", String.valueOf(end_time - start_time), "201"};
